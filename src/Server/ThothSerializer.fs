...
type ThothSerializer
    ...
    interface Serializers.IJsonSerializer with
        member __.Serialize (value, ctx, chunked) =
            if chunked then
                ...
                task {
                    use stream = Lazy<RecyclableMemoryStreamManager>().Value.GetStream ()
        ...
        member __.Deserialize (ctx: HttpContext) = ctx.Request.ReadFromJsonAsync(options).AsTask ()
